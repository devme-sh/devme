...
Eventually devme will manage its own dev workflow (test watch, lint watch, docs serve) once the supervisor is functional — see "Dogfooding" in `CONTEXT.md`.

## Future work

- Disk-based check caching with TTL invalidation (v1 ships with in-memory dedup only)
- Self-update via `devme upgrade` (v1 ships with curl-pipe-sh re-run as the update path)
