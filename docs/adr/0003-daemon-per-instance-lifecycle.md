...
- `devstack up` — start a daemon with the sticky flag set; services survive until `devstack down`.
- `devstack` (no args) — foreground mode; attach the TUI; on TUI exit, ref count drops, daemon shuts down.
...
- Multiple clients can attach simultaneously (TUI in one terminal + `devstack logs` tailing in another + an agent calling `devstack status`). All share the same daemon's view.
...
**No daemon (TUI owns the services).** Matches mprocs. Services die when the TUI quits; `devstack logs` from a second terminal can't reach them. Rejected because `devstack up`/`devstack logs`/agent CLI commands need a process to talk to even when the TUI isn't running.
