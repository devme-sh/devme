...
mod tests {
    ...
    fn envelope_flattens_payload_no_nested_object() {
        ...
        let env = Envelope::new(ClientMessage::Restart { service: "backend".into() });
        let json = serde_json::to_string(&env).unwrap();
    ...
    fn client_message_start_skip_deps_defaults_false() {
        let m: ClientMessage = serde_json::from_str(
            r#"{"kind":"start","service":"backend"}"#,
        )
        .unwrap();
        assert_eq!(m, ClientMessage::Start { service: "backend".into(), skip_deps: false });
    }

    #[test]
    fn client_message_start_with_skip_deps_true() {
        let m: ClientMessage = serde_json::from_str(
            r#"{"kind":"start","service":"backend","skip_deps":true}"#,
        )
        .unwrap();
        assert_eq!(m, ClientMessage::Start { service: "backend".into(), skip_deps: true });
    }
    ...
    fn client_message_round_trips_every_variant() {
        let cases = vec![
            ...
            ClientMessage::Subscribe { services: vec!["a".into(), "b".into()] },
            ClientMessage::Unsubscribe,
            ClientMessage::Restart { service: "backend".into() },
            ClientMessage::Stop { service: "backend".into() },
            ClientMessage::Start { service: "backend".into(), skip_deps: false },
            ClientMessage::Start { service: "backend".into(), skip_deps: true },
    ...
    fn server_message_round_trips_every_variant() {
        ...
        let cases = vec![
            ...
            ServerMessage::Goodbye { reason: "user requested shutdown".into() },
        ];
        for m in cases {
            assert_eq!(json_round_trip(m.clone()), m);
        }
