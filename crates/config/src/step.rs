...
mod tests {
    ...
    fn parse_one(toml_src: &str) -> Step {
        #[derive(Deserialize)]
        struct Wrap { step: indexmap::IndexMap<String, Step> }
...
"#);
        assert_eq!(s.check, "gcloud auth application-default print-access-token >/dev/null");
        assert_eq!(s.provision, Some(Provision::Shell("gcloud auth application-default login".into())));
        assert_eq!(s.trust, Trust::Prompt);
...
"#);
    ...
    fn rejects_unknown_field() {
        let result: Result<Step, _> = toml::from_str(r#"
check = "true"
unknown = 42
"#);
