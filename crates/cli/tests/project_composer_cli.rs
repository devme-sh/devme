...
#[test]
fn generated_file_refresh_is_explicit_in_structured_output() {
    let source = recipe();
    write(
        source.path().join("devme-template.toml"),
        r#"schema_version = 1
...
"#,
    ...
    write(target.join("app.txt"), "generated by a tool\n");
    ...
    assert_eq!(added["regenerated_files"][0], "app.txt");
    assert_eq!(
        fs::read_to_string(target.join("app.txt")).unwrap(),
        "auth = true\n"
    );
